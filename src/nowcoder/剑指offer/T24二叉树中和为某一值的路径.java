...
//		return re;
	...
	private static void accumulate(TreeNode root, int target, ArrayList<ArrayList<Integer>> re, ArrayList<Integer> r) {
		if (root!=null && root.left == null && root.right==null && target==root.val) {
				r.add(root.val);
...
//		r.remove(r.size() - 1);
	...
	private static void accumulate1(TreeNode root, int target, ArrayList<ArrayList<Integer>> re, ArrayList<Integer> r) {
		if (root == null) {
			return;
		}
		r.add(root.val);
